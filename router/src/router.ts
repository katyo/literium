...
const r = Route.new(baseTypes).extra(numTypes).route('/blog/tag-').route({ id: 'nat' }).route('/sort-').extra(orderType).route({ sort: 'ord' });
const post: { id: number, sort: Order } | void = r.match('/blog/tag-12/sort-desc');
const path: string = r.build({ id: 123, sort: Order.Asc });

console.log(post, path);

const ro = r.route('/').route({ other: 'str' });
const posto: { id: number, sort: Order, other: string } | void = ro.match('/blog/tag-3/sort-asc/10');
const patho: string = ro.build({ id: 123, sort: Order.Desc, other: "abc" });

console.log(posto, patho);

const r2 = Route.mix({ r }).add({ ro });

const x/*: Partial<{
    r: { id: number; sort: Order },
    ro: { id: number; sort: Order, other: number },
}> | void*/ = r2.match('/blog/tag-12/sort-desc');

console.log(x && x.ro && x.ro.id);

console.log(r2.match('/blog/tag-12/sort-desc'));
console.log(r2.match('/blog/tag-12/sort-desc/abc'));
console.log(r2.build({ r: { id: 12, sort: Order.Desc } }));
console.log(r2.build({ ro: { id: 12, sort: Order.Desc, other: 12 } }));
