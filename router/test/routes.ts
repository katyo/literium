import { baseTypes, numTypes, TypeApi, Route, route_str, route_arg, route_query, route_or, route_and } from '../src/router';
export { Route };
...
const blog_list = route_and(blog, route_or(
    route_query({ offset: 'nat', count: 'nat' }, numTypes),
    route_query({ count: 'nat' }, numTypes)
));

const blog_search = route_and(blog, route_str('/search'), route_query({ phrase: 'str' }, baseTypes));
